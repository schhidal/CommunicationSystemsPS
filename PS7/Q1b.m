...
phase_offset = pi/4;  % This affects on error rate
c2 = cos(2*pi*fc*t + phase_offset );
...
figure;
plot(1:length(z), z, '.');
title(['Soft Decisions with Phase Offset \gamma = ', num2str(phase_offset), ' Hz']);
...
fprintf('Phase offset: %.4f rad (%.1f degrees)\n', phase_offset, phase_offset*180/pi);
fprintf('Attenuation factor cos(phi): %.4f\n', cos(phase_offset));
